...
PANELS = ['a', 'b']
TITLES = {'a': '(a) phi = 0.94, NH3/air', 'b': '(b) phi = 0.94, NH3/H2/air'}

def load():
    grouped = defaultdict(list)
    with DATA_PATH.open(newline='', encoding='utf-8') as f:
        for row in csv.DictReader(f):
            if row['figure_group'] != 'nh_no_sensitivity':
                continue
...
def main():
    ...
    grouped = load()
    fig, axes = plt.subplots(1, 2, figsize=(7, 4.2), dpi=180, sharex=True)
    for ax, panel in zip(axes, PANELS):
        ...
        ax.invert_yaxis(); ax.set_xlim(-0.32, 0.28)
        ax.set_title(TITLES[panel], fontsize=10, fontweight='bold')
        ax.tick_params(axis='x', labelsize=8)
    axes[0].set_xlabel('Normalized NO sensitivity', fontsize=10)
    axes[1].set_xlabel('Normalized NO sensitivity', fontsize=10)
